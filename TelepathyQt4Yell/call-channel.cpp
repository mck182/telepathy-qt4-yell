...
void CallStream::Private::introspectMainProperties(CallStream::Private *self)
{
    ...
    QDBusPendingCallWatcher *watcher =
        new QDBusPendingCallWatcher(
                self->properties->GetAll(TP_QT4_YELL_IFACE_CALL_STREAM),
                parent);
...
{
    if (!contact) {
        return new Tp::PendingFailure(TP_QT4_YELL_ERROR_INVALID_ARGUMENT,
...
void PendingCallContent::onContentRemoved(const CallContentPtr &content)
{
    ...
    if (mPriv->content == content) {
        ...
        setFinishedWithError(TP_QT4_YELL_ERROR_CANCELLED,
    ...
    QDBusPendingCallWatcher *watcher =
        new QDBusPendingCallWatcher(
                self->properties->GetAll(
                    QLatin1String(TP_QT4_YELL_IFACE_CALL_CONTENT)),
                parent);
    parent->connect(watcher,
            SIGNAL(finished(QDBusPendingCallWatcher*)),
            SLOT(gotMainProperties(QDBusPendingCallWatcher*)));
      ...
      localHoldStateReason(Tp::LocalHoldStateReasonNone)
{
    ...
    Tp::ReadinessHelper::Introspectable introspectableLocalHoldState(
        QSet<uint>() << 0,                                                         // makesSenseForStatuses
        Tp::Features() << Channel::FeatureCore,                                    // dependsOnFeatures (core)
        QStringList() << TP_QT4_IFACE_CHANNEL_INTERFACE_HOLD,                      // dependsOnInterfaces
...
void CallChannel::Private::introspectContents(CallChannel::Private *self)
{
    ...
    QDBusPendingCallWatcher *watcher =
        new QDBusPendingCallWatcher(
                self->properties->GetAll(
                    QLatin1String(TP_QT4_YELL_IFACE_CHANNEL_TYPE_CALL)),
                parent);
    parent->connect(watcher,
            SIGNAL(finished(QDBusPendingCallWatcher*)),
            SLOT(gotMainProperties(QDBusPendingCallWatcher*)));
...
Tp::ChannelClassSpec CallChannel::callClassSpec(const QVariantMap &additionalProperties)
{
    static Tp::ChannelClassSpec spec;

    if (!spec.isValid()) {
        spec = Tp::ChannelClassSpec(TP_QT4_YELL_IFACE_CHANNEL_TYPE_CALL,
    ...
    if (additionalProperties.isEmpty()) {
        ...
        return Tp::ChannelClassSpec(spec, additionalProperties);
    }
}
...
void CallChannel::registerWithChannelFactory(const Tp::ChannelFactoryPtr &channelFactory)
{
    channelFactory->setSubclassFor<Tpy::CallChannel>(callClassSpec());
}
...
Tp::LocalHoldState CallChannel::localHoldState() const
{
    ...
    } else if (!hasInterface(TP_QT4_IFACE_CHANNEL_INTERFACE_HOLD)) {
 ...
 * with error code #TP_QT4_YELL_ERROR_NOT_AVAILABLE.
 ...
 * #TP_QT4_YELL_ERROR_NOT_IMPLEMENTED.
 *
...
{
    if (!hasInterface(TP_QT4_IFACE_CHANNEL_INTERFACE_HOLD)) {
        qWarning() << "CallChannel::requestHold() used with no hold interface";
        return new Tp::PendingFailure(TP_QT4_YELL_ERROR_NOT_IMPLEMENTED,
