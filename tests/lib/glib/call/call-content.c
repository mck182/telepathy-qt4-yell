...
static void
content_remove (FutureSvcCallContent *iface,
    guint reason,
    const gchar *detailedReason,
    const gchar *message,
    DBusGMethodInvocation *context)
{
  ExampleCallContent *self = EXAMPLE_CALL_CONTENT (iface);

  example_call_stream_close (self->priv->stream);

  future_svc_call_content_return_from_remove (context);
}

static void
content_iface_init (gpointer iface, gpointer data)
{
